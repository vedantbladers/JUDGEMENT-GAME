...
## 15. Premature Activation & Persistent Glow on Incomplete Lobby Codes (Validation & UX)
**Symptom:** 
- In the lobby screen (`/lobby`), the "Join Match" button became enabled and clickable after typing only 4 characters, despite room codes strictly requiring 6 alphanumeric characters generated by the backend.
- Additionally, even with 0 to 3 characters typed, the button retained an active glowing purple gradient background and shadow, misleading users into thinking the button was active or that shorter codes were valid.
**Root Cause:** 
- **Premature Length Threshold:** The button was configured with `disabled={joinCode.length < 4}`, allowing premature submissions of 4 or 5 characters that would inevitably fail on the backend.
- **Unconditioned Glow Classes:** The button markup permanently included active visual tokens (`bg-gradient-to-r from-indigo-600 to-purple-600 shadow-lg shadow-indigo-500/20`), combined with a naive `disabled:opacity-50` that still projected a colorful glow rather than an unlit disabled state.
**Resolution:** 
- Strictly enforced `disabled={joinCode.trim().length !== 6}` on the button and added defensive 6-character validation inside `handleJoin`.
